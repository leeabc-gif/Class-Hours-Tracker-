-- ============================================================
-- v1.0.9 · AI 中转平台 + 通知公告 + 课表导入
--
-- 新建 7 张表：
--   ks_ai_channel    AI 渠道（多厂商上游）
--   ks_ai_model      AI 模型与计费倍率
--   ks_ai_token      教师 API 令牌（sk-，供外部调用）
--   ks_ai_quota      教师 AI 额度（总余额 + 日/周/月周期限额）
--   ks_ai_usage_log  AI 用量日志（计费与审计依据）
--   ks_notice        通知公告
--   ks_notice_read   公告已读回执
--
-- 并放宽 ks_lesson.source 枚举，新增 'import'（课程表导入来源）。
--
-- 全部使用 CREATE TABLE IF NOT EXISTS，可重复执行（老站点在线升级安全）。
-- ============================================================

-- ① 渠道：多厂商上游（OpenAI / Claude / 通义 / DeepSeek / 自定义 …）
...
-- ⑧ 关于 ks_lesson.source 补充说明
--    该列实为 varchar(16)（非 enum），'import'（课程表导入来源）可直接写入，
--    无需 ALTER 改表。仅同步 install.sql 的列注释，让来源语义完整。
